...
export async function executeRoutes(fastify) {
  ...
  // AUCUN routage IA, contrairement à execute-auto. Le plugin/commande
  // demandés sont exactement ceux exécutés, zéro incertitude de sélection.
  // ----------------------------------------------------------
  fastify.post('/v1/actions/execute-command', {
    ...
    try {
      ...
      let promptBody
      try {
        promptBody = await pluginRouter.loadCommandContent(plugin, command, parameters)
      } catch (err) {
      ...
      request.log.info({ task_id: taskId, plugin, command }, '[ExecuteCommand] Tâche démarrée')
      ...
      reply.code(202).send({
        ...
        action_id: actionId,
        status:    'in_progress',
        steps:     [{ plugin, agent: command, skills: [], reason: 'Commande choisie manuellement — pas de routage IA.' }],
      })
      ...
      ;(async () => {
        ...
        try {
          ...
          await withOrgScope(bgDb, org_id, () => bgDb.query(
            ...
            [output, JSON.stringify([{ plugin, agent: command, skills: [], reason: '', model: provider, output, tool_calls: toolCalls }]), taskId, org_id]
          ))
